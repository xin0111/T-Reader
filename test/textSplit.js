...
{
    var cc = stringify(sentence).split('\n');
    var ccc = stringify(sentence).match(splitAllChars).filter(w => w.length);
    // merge(stringify(sentence).match(splitWords).map(d => {
    //     if (noSpaceLanguage.test(d)) {
    //         var cccc = d.match(splitAllChars);
    //         var ac = 0;
    //     }
    // }));
    if (!noSpaceLanguage.test(sentence))
        return stringify(sentence).match(splitWords).filter(w => w.length);
