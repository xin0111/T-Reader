...
function _csResponse(dfd)
{
    return function() {
        ...
        if (!err)
            ...
            console.log("chromeStorage error: '%s'", err.message);
...
_(Wrapper.prototype).extend({
    ...
    getQuotaObject : function() {
        return _(this.storage).pick('QUOTA_BYTES', 'QUOTA_BYTES_PER_ITEM', 'MAX_ITEMS', 'MAX_SUSTAINED_WRITE_OPERATIONS_PER_MINUTE', 'MAX_WRITE_OPERATIONS_PER_HOUR');
    }
});
...
function ChromeStorage(name, type)
{
    ...
    this.loaded = this.store.get(this.name).pipe(this._parseRecords).done((function(records) {
...
_(ChromeStorage.prototype).extend({
    ...
    destroy : function(model) {
        return this.store.remove(this._idOf(model), this._destroyed.bind(this, model)).pipe(wantsJSON(model));
    },
...
Backbone.ChromeStorage.sync = Backbone.chromeSync = function(method, model, options, error) {
    var store = model.chromeStorage || model.collection.chromeStorage, resp, isFn = _.isFunction;
